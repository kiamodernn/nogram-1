...
public class DialogCellLock extends BaseCell {
    ...
    private boolean isLockedd = false ;

    public DialogCellLock(Context context) {
        super(context);
        Theme.createDialogsResources(context);
        avatarImage.setRoundRadius(AndroidUtilities.dp(26));
    }

    public void setDialog(TLRPC.TL_dialog dialog, int i, int type , boolean isLocked) {
...
//        return null;
    ...
    public static ArrayList<TLRPC.TL_dialog> getFaveList() {
        ...
        try {
            List<Integer> ids = ApplicationLoader.databaseHandler.getAllFavouritesIdByUser() ;
//        Log.e("log" ,"size ids " + ids.size() +"-") ;
            if(ids.size() <=0){
                return  dialogsFave ;
            }
...
//            Log.e("log" , "id dialog" + d.id +"-") ;
                if(ids.contains((int)d.id)){
//            Log.e("log" , "id dialog fave" + d.id +"-") ;
                    dialogsFave.add(d) ;
                }
            }
        }catch (Exception e){
