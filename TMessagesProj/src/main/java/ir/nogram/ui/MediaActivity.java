...
import ir.nogram.messanger.query.SharedMediaQuery;
...
import ir.nogram.ui.ActionBar.ActionBarPopupWindow;
...
import ir.nogram.ui.Cells.SharedDocumentCell;
import ir.nogram.ui.Cells.SharedLinkCell;
import ir.nogram.ui.Cells.SharedMediaSectionCell;
import ir.nogram.ui.Cells.SharedPhotoVideoCell;
...
public class MediaActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate, PhotoViewer.PhotoViewerProvider {
    ...
    public View createView(Context context) {
            ...
            public void onItemClick(int id) {
                if (id == -1) {
                    ...
                    for (int a = 1; a >= 0; a--) {
                        ...
                        for (Integer idd : ids) {
                            if (idd > 0) {
                                fmessages.add(selectedFiles[a].get(idd));
                                //                                selectedFiles[a].remove(selectedFiles[a].get(idd).getId());
                            }
                        }
                    }
                    Collections.sort(fmessages, new Comparator<MessageObject>() {
                        @Override
                    ...
                    Dialog dialog = new ShareAlertMulti(getParentActivity(), fmessages, null, ChatObject.isChannel(currentChat) && !currentChat.megagroup && currentChat.username != null && currentChat.username.length() > 0, null);
                    showDialog(dialog);
                }
        ...
        dropDown.setTypeface(AndroidUtilities.getTypeface(ThemeManager.getFont()));
        ...
        if ((int) dialog_id != 0) {
            actionModeViews.add(actionMode.addItem(direct_multi_forward, R.drawable.ic_direct_forward, AndroidUtilities.dp(54)));
            actionModeViews.add(actionMode.addItem(quoteforward, R.drawable.ic_ab_fwd_quoteforward , AndroidUtilities.dp(54)));
            actionModeViews.add(actionMode.addItem(forward, R.drawable.ic_ab_fwd_forward_without, AndroidUtilities.dp(54)));
