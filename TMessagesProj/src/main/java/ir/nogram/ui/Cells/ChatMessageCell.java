...
import ir.nogram.ui.Components.RadialProgress;
import ir.nogram.ui.Components.RoundVideoPlayingDrawable;
import ir.nogram.ui.Components.SeekBar;
import ir.nogram.ui.Components.SeekBarWaveform;
import ir.nogram.ui.Components.StaticLayoutEx;
...
public class ChatMessageCell extends BaseCell implements SeekBar.SeekBarDelegate, ImageReceiver.ImageReceiverDelegate, MediaController.FileDownloadProgressListener {
    ...
    public interface ChatMessageCellDelegate {
        void didLongPressedUserAvatar(ChatMessageCell cell, TLRPC.User user);

        void didPressedFave(ChatMessageCell cell);
    ...
    protected void onLongPress() {
        ...
        if (delegate != null && !avatarPressed) {
            delegate.didLongPressed(this);
        }
        if (delegate != null && avatarPressed) {
            delegate.didLongPressedUserAvatar(this, currentUser);
//            delegate.didLongPressed(this);
        }
    ...
    public void setMessageObject(MessageObject messageObject, boolean bottomNear, boolean topNear) {
        setMessageObject(messageObject, bottomNear, topNear, 0);
    }

    public void setMessageObject(MessageObject messageObject, boolean bottomNear, boolean topNear, final int justFrom) {
        ...
        if (messageChanged || dataChanged || isPhotoDataChanged(messageObject) || pinnedBottom != bottomNear || pinnedTop != topNear) {
            ...
            if (messageObject.type == 0) {
                ...
                setMessageObjectInternal(messageObject, justFrom);
                ...
                if (hasLinkPreview || hasGamePreview || hasInvoicePreview) {
                    ...
                    if (documentAttachType != DOCUMENT_ATTACH_TYPE_MUSIC && documentAttachType != DOCUMENT_ATTACH_TYPE_AUDIO && documentAttachType != DOCUMENT_ATTACH_TYPE_DOCUMENT) {
                        ...
                        if (hasInvoicePreview) {
                            ...
                            stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface(ThemeManager.getFont())), 0, price.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            ...
            //HoseinKord
            if (justFrom > 0) {
                if (messageObject.messageOwner.from_id != justFrom) {
                    totalHeight = 0;
                }
            }
            botButtons.clear();
...
//
    ...
    private void measureTime(MessageObject messageObject) {
        ...
        if ((messageObject.messageOwner.flags & TLRPC.MESSAGE_FLAG_HAS_VIEWS) != 0) {
            currentViewsString = String.format("%,d", Math.max(1, messageObject.messageOwner.views));
...
//            currentViewsString = String.format("%s", LocaleController.formatShortNumber(Math.max(1, messageObject.messageOwner.views), null));
        ...
        if (authorName || viaBot) {
            ...
            if (viaBot) {
                ...
                if (currentNameString.length() > 0) {
                    ...
                    stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface(ThemeManager.getFont()), 0, color), nameStringFinal.length() + 5, stringBuilder.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                    ...
                    stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface(ThemeManager.getFont()), 0, color), 4, stringBuilder.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                    nameStringFinal = stringBuilder;
                }
        ...
        if (drawForwardedName && messageObject.isForwarded()) {
            ...
            if (currentForwardUser != null || currentForwardChannel != null) {
                ...
                if (viaString != null) {
                    ...
                    stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface(ThemeManager.getFont())), stringBuilder.length() - viaUsername.length() - 1, stringBuilder.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                } else {
                    stringBuilder = new SpannableStringBuilder(String.format("%s %s", fromString, name));
                }
                stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface(ThemeManager.getFont())), fromString.length() + 1, fromString.length() + 1 + name.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    private void setMessageObjectInternal(MessageObject messageObject, int justFrom) {
        if ((messageObject.messageOwner.flags & TLRPC.MESSAGE_FLAG_HAS_VIEWS) != 0) {
            if (currentMessageObject.isContentUnread() && !currentMessageObject.isOut()) {
                MessagesController.getInstance().addToViewsQueue(currentMessageObject.messageOwner, false);
                currentMessageObject.setContentIsRead();
