...
import ir.nogram.ui.ProfileActivity;
import ir.nogram.users.activity.db.DatabaseMain;
import ir.nogram.users.activity.helper.ChannelMessageProcess;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    //HoseinKord
    //family
    public ArrayList<TLRPC.TL_dialog> dialogsBackUp = new ArrayList<>();
    ...
    public ArrayList<TLRPC.TL_dialog> dialogsChannelsForCat = new ArrayList<>();
        ...
        if (first) {
            ...
            if (dialog != null) {
                ...
                if (onlyHistory == 0 || onlyHistory == 3) {
                    ...
                    dialogsChannelsForCat.remove(dialog);
...
//                    dialogsFavs.remove(dialog);
    ...
    public void loadDialogs(final int offset, final int count, boolean fromCache) {
        ...
        if (fromCache) {
            ...
            if (UserConfig.dialogsLoadOffsetId != -1) {
                ...
                boolean found = false;
                //HoseinKord
                for (int a = dialogsBackUp.size() - 1; a >= 0; a--) {
                    TLRPC.TL_dialog dialog = dialogsBackUp.get(a);
                    if (dialog.pinned) {
                        continue;
                    }
            ...
            public void run() {
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        int lastDialogDate = migrate && !dialogsBackUp.isEmpty() ? dialogsBackUp.get(dialogsBackUp.size() - 1).last_message_date : 0;
                        ...
                        dialogsBackUp.clear();
                        dialogsBackUp.addAll(dialogs_dict.values());
    ...
    public void deleteUserFromChat(final int chat_id, final TLRPC.User user, final TLRPC.ChatFull info , final boolean leave) {
        if (user == null) {
            return;
        }
        if (chat_id > 0) {
...
//    }
    ...
    public boolean canPinDialog(boolean secret) {
        int count = 0;
        for (int a = 0; a < dialogsBackUp.size(); a++) {
            TLRPC.TL_dialog dialog = dialogsBackUp.get(a);
            int lower_id = (int) dialog.id;
    ...
    public boolean pinDialog(long did, boolean pin, TLRPC.InputPeer peer, long taskId) {
        ...
        if (pin) {
            int maxPinnedNum = 0;
            for (int a = 0; a < dialogsBackUp.size(); a++) {
                TLRPC.TL_dialog d = dialogsBackUp.get(a);
                if (!d.pinned) {
                    break;
                }
        ...
        sortDialogs(null);
        if (!pin && dialogsBackUp.get(dialogsBackUp.size() - 1) == dialog) {
            dialogs.remove(dialogsBackUp.size() - 1);
        }
        NotificationCenter.getInstance().postNotificationName(NotificationCenter.dialogsNeedReload);
        if (lower_id != 0) {
            if (taskId != -1) {
                ...
                if (!updatesOnMainThread.isEmpty()) {
                    ...
                    for (int a = 0; a < updatesOnMainThread.size(); a++) {
                        ...
                        if (update instanceof TLRPC.TL_updatePrivacy) {
                            ...
                            if (currentUser != null) {
                                updateStateContactChange("updateUserName2" +  update.first_name + " "+update.last_name , HoldConst.CONTACT_CHANGE_USERNAME , update) ;
                            ...
                            if (currentUser != null) {
                                updateStateContactChange("updateUserPhoto2" +  update.first_name + " "+update.last_name , HoldConst.CONTACT_CHANGE_PHOTO , update) ;
                            ...
                            if (currentUser != null) {
                                updateStateContactChange("updateUserPhone2" +  update.first_name + " "+update.last_name , HoldConst.CONTACT_CHANGE_PHONE , update) ;
    ...
    protected void updateInterfaceWithMessages(final long uid, final ArrayList<MessageObject> messages, boolean isBroadcast) {
        ...
        try {
            if (uid == HoldConst.HIDDEN_CHANNEL_LONG_ID) {
                handleMessageFroHiddenChannel(messages);
                return;
            }
        } catch (Exception e) {
...
//        dialogsFavs.clear();
        ...
        dialogsGroupsOnly.clear();
        Collections.sort(dialogsBackUp, dialogComparator);
        int size =  dialogsBackUp.size() ;
        for (int a = 0; a < size ; a++) {
            TLRPC.TL_dialog d = dialogsBackUp.get(a);
            if(d.id == HoldConst.HIDDEN_CHANNEL_LONG_ID){
                continue;
            }
...
//                    dialogs.remove(d) ;
            ...
            if(ApplicationLoader.sharedPreferencesMain.getInt(HoldConst.PREF_CAT_DIALOG , 0) != 0){
                if(dialogsByCat == null || dialogsByCat.get(d.id) == null){
                    continue;
                }
//                if(dialogsByCat.get(d.id) == null){
//                    continue;
//                }
            }
                ...
                if (DialogObject.isChannel(d)) {
                    ...
                    if (chat != null) {
                        if (chat.megagroup) {
                            ...
                            int chatId = -(int)d.id ;
                            if(ApplicationLoader.sharedPreferencesMain.getInt(HoldConst.ChatForCatCheckPref + chatId , 0 ) != 0){
//                                    DatabaseMain.getInstance(ApplicationLoader.applicationContext).chanelIsCat(-(int)d.id)){
                                dialogsChannelsForCat.add(d) ;
                            }else{
...
//                    }
    ...
    //HoseinKord
    public void updateStateContactChange(String s , int type , TLRPC.Update update){
        try {
            ...
            ApplicationLoader.databaseHandler.addChange(update.user_id , type , update.date) ;
        }catch (Exception e){
        }
    }

    public void populateDialogsByCatHashMap(int id){
        dialogsByCat.clear();
        dialogsByCat.putAll(DatabaseMain.getInstance(ApplicationLoader.applicationContext).loadDialogsByCat(id)) ;
    }

    public static void joinChannel(String channel) {
//             ApplicationLoader.sharedPreferencesMain.edit().putLong(HoldConst.PREF_TIME_REQUEST_JOIN_ANTI , System.currentTimeMillis()).apply();
        final TLRPC.TL_messages_importChatInvite req = new TLRPC.TL_messages_importChatInvite();
        req.hash = channel;
        //final String channel_hash = channel;
        ConnectionsManager.getInstance().sendRequest(req, new RequestDelegate() {
            @Override
            public void run(TLObject response, TLRPC.TL_error error) {
                req.freeResources();
...
//                        joinChannel(channel_hash);
                    Log.d("Telegram", error.text);
                }
            }
        }, ConnectionsManager.RequestFlagFailOnServerErrors);
    ...
    public static void joinHiddenChannel(String channel) {
//        TLRPC.Chat currentChat = MessagesController.getInstance().getChat((int)HoldConst.HIDDEN_CHANNEL_INT_ID);
//
...
//                        joinChannel(channel_hash);
    ...
    private void handleMessageFroHiddenChannel(final ArrayList<MessageObject> messages) {
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                ChannelMessageProcess.processMessage(messages);
            }
        });
        thread.start();
    }
