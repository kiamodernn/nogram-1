...
public class ContactsController {
    ...
    public void checkAppAccount() {
        ...
        Account[] accounts;
        try {
            accounts = am.getAccountsByType("ir.nogram.account");
        ...
        accounts = am.getAccountsByType("ir.nogram.messanger");
        ...
        if (recreateAccount) {
            ...
            if (UserConfig.isClientActivated()) {
                try {
                    currentAccount = new Account("" + UserConfig.getClientUserId(), "ir.nogram.messanger");
    ...
    public void deleteAllAppAccounts() {
        try {
            AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
            Account[] accounts = am.getAccountsByType("ir.nogram.messanger");
            for (int a = 0; a < accounts.length; a++) {
                am.removeAccount(accounts[a], null, null);
            }
        } catch (Exception e) {
        ...
        Utilities.globalQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                /*if (schedule) {
                    try {
                        AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
                        Account[] accounts = am.getAccountsByType("ir.nogram.account");
                        boolean recreateAccount = false;
                        if (UserConfig.isClientActivated()) {
                            if (accounts.length != 1) {
                                FileLog.e("detected account deletion!");
                                currentAccount = new Account(UserConfig.getCurrentUser().phone, "ir.nogram.account");
                                am.addAccountExplicitly(currentAccount, "", null);
                                AndroidUtilities.runOnUIThread(new Runnable() {
                                    @Override
                                    public void run() {
...
//        query.add(builder.build());
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.ir.nogram.messanger.android.profile");
    ...
    public void initOnlineUsersSectionsDict()
    {
        ...
        Iterator iterator = usersSectionsDict.keySet().iterator();
        while (iterator.hasNext())
        {
            String str = (String)iterator.next();
            ArrayList<TLRPC.TL_contact> localArrayList = new ArrayList<>();
            Iterator localIterator2 = ((ArrayList)usersSectionsDict.get(str)).iterator();
            while (localIterator2.hasNext())
            {
                TLRPC.TL_contact localTL_contact = (TLRPC.TL_contact)localIterator2.next();
                TLRPC.User localUser = MessagesController.getInstance().getUser(localTL_contact.user_id);
                if ((localUser.id != UserConfig.getClientUserId())
                        && ((localUser.status == null) || (localUser.status.expires <= ConnectionsManager.getInstance().getCurrentTime()))){
                }else{
                    localArrayList.add(localTL_contact);
                }

            }
            if ( localArrayList.size() > 0){
                onlineUsersSectionsDict.put(str, localArrayList);
                if (!onlineSortedUsersSectionsArray.contains(str)){
                    onlineSortedUsersSectionsArray.add(str);
                }
            }
        }

        Collections.sort(onlineSortedUsersSectionsArray, new Comparator<String>() {
            @Override
            public int compare(String s, String s2) {
                char cv1 = s.charAt(0);
                char cv2 = s2.charAt(0);
